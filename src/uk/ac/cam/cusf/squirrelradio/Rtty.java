...
public class Rtty {
    ...
    // Generates a tone of particular frequency (1 bit long at specified baud rate)
    ...
    // Takes string and returns RTTY audio sample, complete with start and stop bits
    ...
    public File createRtty(Location location, Intent battery) {
    	...
    	if (battery != null) {
	    	...
	    	String secure = String.format("%.1f",(100.0*level)/scale);
	    	secure += ","+String.valueOf(temperature);
	    	secure += ","+String.valueOf(health);
	    	
	    	encryptor.setKeyPosition(realtime);
	    	msg += ","+encryptor.encrypt(secure);
    	} else {
    		msg += ",";
    	}
    	*/
