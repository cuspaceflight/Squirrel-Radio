...
public class Sstv {
    ...
    public File generateImage() {
        ...
        if (image == null) {
            // Either the JPG couldn't be opened, or there isn't a new one in
            // the directory since last time
    ...
    private int rowLength() {
        ...
        int gapLength = 4 * 2 * (int) (SAMPLE_RATE * GAP_TIME *
                Math.pow(10, -6));
        int pixelLength = WIDTH * 3 * 2
                * (int) (SAMPLE_RATE * PIXEL_TIME * Math.pow(10, -6));
    ...
    private void pixelSample(int lum, BufferedOutputStream output)
            throws IOException {
        generateSample(pixelFreq(lum), PIXEL_TIME, output);
    }

    private void generateSample(int freq, int len, BufferedOutputStream output)
            throws IOException {
    ...
    private int pixelFreq(int lum) {
        return (int) (BLACK_FREQ + (WHITE_FREQ - BLACK_FREQ)
                * (1.0 * lum / 255));
    }
